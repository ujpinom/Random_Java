...
public class FallaLineaALineaTierraEnLinea {
	...
	public FallaLineaALineaTierraEnLinea(double[][] matrizSecuencia0, double[][] matrizSecuencia1,
		...
		double [][]zBarrafinal1 = zb1.inserccionEntreBarras(matrizInserccionEntreDosBarras1, barras.indexOf(elementoFallado.getBarra1()), barras.indexOf(elementoFallado.getBarra2()));
		double [][]zBarrafinal2 = zb2.inserccionEntreBarras2(matrizInserccionEntreDosBarras2, barras.indexOf(elementoFallado.getBarra1()), barras.indexOf(elementoFallado.getBarra2()));
		double [][]zBarrafinal0 = zb0.inserccionEntreBarras0(matrizInserccionEntreDosBarras0, barras.indexOf(elementoFallado.getBarra1()), barras.indexOf(elementoFallado.getBarra2()));
		...
		for (int i=0;i<barras.size();i++) {
			...
			if(elementoFallado.getBarra1()==barras.get(i)||elementoFallado.getBarra2()==barras.get(i) ) {
				...
				try {
					...
					Complejo iA0= Complejo.cociente(new Complejo(vectorVoltajesSecuencia0.get(i-1)-vA0Imaginario.modulo(),0), new Complejo(0,elementoFallado.getimpedanciaLineaZ0()/2));
					Complejo iA2=Complejo.cociente(new Complejo(vectorVoltajesSecuencia2.get(i-1)-vA2Imaginario.modulo(),0), new Complejo(0,elementoFallado.getimpedanciaLineaZ2()/2));
					Complejo iA1= Complejo.cociente(new Complejo(vectorVoltajesSecuencia1.get(i-1)-vA1Imaginario.modulo(),0), new Complejo(0,elementoFallado.getimpedanciaLineaZ1()/2));
	...
	public void contribucionMaquina() {
		...
		for (int i=0;i<generador.size();i++) {
			...
			try {
				...
				Complejo iA0=Complejo.cociente(Complejo.producto(new Complejo(-1,0), Complejo.polar2Cartesiano(vectorVoltajesSecuencia0.get(indexBarraGe-1), generador.get(i).getBarra().getAnguloVoltajeSecuencia0())), new Complejo(0,generador.get(i).getImpedanciaZ0()+generador.get(i).getImpedanciaAterrizamiento())) ;
	...
	public void corrienteLineas() {
		...
		for(int i=0;i<lineas.size();i++) {
			...
			try {
				...
				Complejo iA0= Complejo.cociente(new Complejo((vectorVoltajesSecuencia0.get(minimo-1)-vectorVoltajesSecuencia0.get(maximo-1)),0), new Complejo(0,lineas.get(i).getimpedanciaLineaZ0()));
				Complejo iA1;
		...
		for(int i=0;i<trafo.size();i++) {
			...
			try {
				...
				if((trafo.get(i).getConexionPrimaria().contains("Y-")||trafo.get(i).getConexionSecundaria().contains("Y-"))||(trafo.get(i).getConexionPrimaria().contains("DELTA")&&trafo.get(i).getConexionSecundaria().contains("DELTA"))) {
					
					iA0=new Complejo(0,0);
				}
				else {
					iA0= Complejo.cociente(new Complejo((vectorVoltajesSecuencia0.get(minimo-1)-vectorVoltajesSecuencia0.get(maximo-1)),0), new Complejo(0,trafo.get(i).getimpedanciaLineaZ0()+3*trafo.get(i).getImpedanciaAterrizamientoPrimaria()+
	...
	public void vectorVoltajes() {
		...
		vA1Imaginario=  Complejo.resta(new Complejo(elementoFallado.getTensionLineaPuntoFalla(),0),Complejo.producto(new Complejo(0,matrizImpedanciaZ1[matrizImpedanciaZ2.length-1][matrizImpedanciaZ2.length-1]),corrientePuntoFallaSecuencia1));
	...
	public void corrientePuntoFalla() {
		...
		try {
			...
			double impedanciTotal= matrizImpedanciaZ1[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+((matrizImpedanciaZ2[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]*
					(matrizImpedanciaZ0[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+
					3*elementoFallado.getImpedanciaFalla()))/(matrizImpedanciaZ2[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+matrizImpedanciaZ0[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+3*elementoFallado.getImpedanciaFalla()));
			...
			impedanciTotal=(matrizImpedanciaZ0[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+
					3*elementoFallado.getImpedanciaFalla())/(matrizImpedanciaZ2[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+matrizImpedanciaZ0[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+3*elementoFallado.getImpedanciaFalla());
			corrientePuntoFallaSecuencia2=Complejo.producto(Complejo.producto(new Complejo(-1,0), corrientePuntoFallaSecuencia1), new Complejo(impedanciTotal,0));
			impedanciTotal=matrizImpedanciaZ2[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]/(matrizImpedanciaZ2[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]
					+matrizImpedanciaZ0[matrizImpedanciaZ1.length-1][matrizImpedanciaZ1.length-1]+3*elementoFallado.getImpedanciaFalla());
			corrientePuntoFallaSecuencia0=Complejo.producto(Complejo.producto(new Complejo(-1,0), corrientePuntoFallaSecuencia1), new Complejo(impedanciTotal,0));
			...
			Complejo iB=Complejo.suma(Complejo.suma(Complejo.producto(aCuadrado, corrientePuntoFallaSecuencia1), Complejo.producto(a, corrientePuntoFallaSecuencia2)), corrientePuntoFallaSecuencia0);
			
			elementoFallado.setAngCorrientePuntoFallaFaseB(iB.argumento());
			elementoFallado.setMagcorrientePuntoFallaFaseB(iB.modulo());
			
			Complejo iC=Complejo.suma(Complejo.suma(Complejo.producto(aCuadrado, corrientePuntoFallaSecuencia2), Complejo.producto(a, corrientePuntoFallaSecuencia1)), corrientePuntoFallaSecuencia0);
